...
static inline const char *__clang_happy_fmt_string(const char *_) {
	return _;
}
static inline size_t __smart_string_length(const char *s, size_t otherwise) {
	return (s == NULL) ? otherwise : strlen(s);
}
