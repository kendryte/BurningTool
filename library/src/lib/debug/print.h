...
#define debug_print_win32(fmt, ...)                                                                                                            \
	...
	debug_print_prefix(debug_output, debug_buffer_remain, __FILE__, __LINE__);                                                                 \
	debug_printf(fmt ": %lu - " __VA_OPT__(, ) __VA_ARGS__, GetLastError());                                                                   \
	debug_output_move(FormatMessageA(                                                                                                          \
		FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, GetLastError(), 0, (LPSTR)debug_output, debug_buffer_remain, NULL)); \
	DEBUG_END();
