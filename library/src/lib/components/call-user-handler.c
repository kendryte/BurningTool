...
kburn_err_t global_init_user_handle_thread(KBCTX scope) { return event_thread_init(scope, "userCb", user_callback_thread, &scope->user_event); }

void user_handler_wrap_async(struct user_handler_wrap_data *data) { event_thread_queue(data->device->_scope->user_event, data); }
void user_handler_wrap_sync(struct user_handler_wrap_data *data) { user_callback_thread(NULL, data); }
